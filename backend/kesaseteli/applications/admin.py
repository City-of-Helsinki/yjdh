...
class IsValidSchoolFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "yes":
            return queryset.filter(school__in=School.objects.values("name"))
        if self.value() == "no":
            return queryset.exclude(school__in=School.objects.values("name"))
        return queryset
...
class YouthApplicationAdmin(admin.ModelAdmin):
    ...
    is_valid_school.short_description = _("is in school list")
