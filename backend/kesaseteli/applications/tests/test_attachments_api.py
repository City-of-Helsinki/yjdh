...
def handle_attachment_url(summer_voucher, attachment):
    return reverse(
        "v1:summervoucher-handle-attachment",
        kwargs={"pk": summer_voucher.pk, "attachment_pk": attachment.pk},
    )
...
def _upload_file(
    ...
    with open(
        ...
        return api_client.post(
            reverse(
                "v1:summervoucher-post-attachment", kwargs={"pk": summer_voucher.pk}
            ),
            {
...
def test_invalid_attachment_upload(api_client, summer_voucher, extension):
    tmp_file = tempfile.NamedTemporaryFile(suffix=f".{extension}")
    tmp_file.write(b"invalid data " * 100)
    tmp_file.seek(0)

    response = api_client.post(
        reverse("v1:summervoucher-post-attachment", kwargs={"pk": summer_voucher.pk}),
        {
...
@pytest.mark.django_db
def test_attachment_upload_too_big(api_client, summer_voucher):
...
def test_get_attachment(request, api_client, summer_voucher, attachment_type):
    ...
    attachment_url = handle_attachment_url(summer_voucher, attachment)
...
def test_delete_attachment(request, api_client, summer_voucher, attachment_type):
    _upload_file(request, api_client, summer_voucher, "pdf", attachment_type)
    attachment = Attachment.objects.first()
    attachment_delete_url = handle_attachment_url(summer_voucher, attachment)
...
@pytest.mark.django_db
def test_delete_attachment_when_not_saved(api_client, summer_voucher):
    employment_contract_url = reverse(
        "v1:summervoucher-handle-attachment",
        kwargs={"pk": summer_voucher.pk, "attachment_pk": uuid.uuid4()},
    )
