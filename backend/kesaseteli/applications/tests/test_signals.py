...
# ---------------------------------------------------------------------------
# User deletion / inactivation / activation
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_user_deletion_does_not_anonymize_actor_name():
    """Deleting a user does not change actor_name on related logs."""
    actor = HandlerUserFactory(first_name="Maija", last_name="Meikäläinen")
    app = YouthApplicationFactory(status="submitted")
    ...
    assert log.actor_name == "Maija Meikäläinen"  # actor_name is unchanged
...
@pytest.mark.django_db
def test_user_inactivation_does_not_affect_timeline():
    """Inactivating a user does not change timeline logs."""
    actor = HandlerUserFactory(first_name="Matti", last_name="Meikäläinen")
    app = YouthApplicationFactory(status="submitted")
    TimelineActivityLog.objects.create(
        application_type="youthapplication",
        application_id=app.pk,
