...
This app provides utilities to automatically manage a configurable admin user group (default `kesaseteli-admin`) and simplify permission setup for staff users in **development environments**.
...
**What it does:**
1.  Creates or retrieves the admin group (default `kesaseteli-admin`).
...
- **Action**: Checks if the user is newly created and has `is_staff=True`. If so, it adds them to the admin group (default `kesaseteli-admin`).
