...
- [Documentation](#documentation)
  ...
  - [Management Commands](#management-commands)
    ...
    - [Ensure Talpa Export Test Applications](#ensure-talpa-export-test-applications)
...
* Create `.env.kesaseteli-backend` file: `touch .env.kesaseteli-backend`. An example can be found from monorepo root directory in `.env.kesaseteli-backend.example`. For local development, `.env` is also accepted when the named file is absent.
...
* Run `uv run manage.py ensure_email_templates` (populates email templates)
* Run `uv run manage.py setup_admin_permissions` (sets up admin permissions)
...
* Run `uv run manage.py makemessages --ignore .venv --no-location -l fi -l sv -l en`
* Run `uv run manage.py compilemessages --ignore .venv`
...
Production images use `uv sync --locked --no-dev --group prod`.

Run Python commands through `uv run`, for example `uv run pytest` or
`uv run manage.py shell_plus`. No virtual environment activation is needed.

## Code format

This project uses [Ruff](https://docs.astral.sh/ruff/) for code formatting and quality checking.
The pre-commit hooks install their own pinned Ruff version. To use Ruff without
the hooks, install it separately with `uv tool install ruff`.
...
`uv run manage.py create_summervoucher_configuration`
...
#### Ensure Talpa Export Test Applications

`python manage.py ensure_talpa_export_test_applications --count <count>`

Ensures at least `count` `EmployerSummerVoucher` records are available in the
Talpa export queue (`unhandled`, `for_export`). If the current count is already
at or above the target, the command does nothing. If fewer exist, it creates
clearly labelled dummy applications (names contain `"Autogenerated"` and a
timestamp) to make up the difference.

**Arguments:**

*   `--count` *(required)*: Target number of Talpa-exportable vouchers.

**Intended use:**

This command is designed for **dev and test environments only**. It is meant to
be run by an OpenShift/Kubernetes CronJob (e.g. every 15 minutes) so that the
Talpa payment-platform team always has data to test against. The scheduling and
the environment restriction are enforced at the CronJob manifest level — the
command itself performs no environment check.

**Example:**

```bash
python manage.py ensure_talpa_export_test_applications --count 5
```
