...
class InstalmentSerializer(serializers.ModelSerializer):
    ...
    def validate_due_date(self, due_date):
        if self.instance.instalment_number == 2:
            ...
            instalment_1_qs = Instalment.objects.filter(
                calculation=calculation, instalment_number=1
            )
            if instalment_1_qs:
                ...
                if instalment_1.due_date >= due_date:
                    raise serializers.ValidationError(
                        {
                            "due_date": _(
                                "Due date cannot be before first instalment due date"
                            )
                        }
                    )
