...
def test_talpa_lines(talpa_service):
    csv_lines = talpa_service.get_talpa_lines()
...
def test_talpa_csv_output(talpa_service):
    csv_lines = _split_csv(talpa_service.get_talpa_csv_string())
    assert csv_lines[0][0] == '"Application number"'
    for idx, col in enumerate(TalpaService.TALPA_COLUMNS):
...
def test_talpa_csv_delimiter(talpa_service_with_one_application):
    ...
    application.save()
    assert ';"test;12";' in talpa_service_with_one_application.get_talpa_csv_string()
...
def test_write_talpa_csv_file(talpa_service, tmp_path):
    ...
    talpa_service.write_talpa_csv_file(output_file)
