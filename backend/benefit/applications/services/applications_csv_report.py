...
from applications.services.csv_export_base import CsvColumn, get_organization_type, CsvExportBase, nested_queryset_attr
...
def get_export_notes(application):
    notes = []
    if application.de_minimis_aid_set.count() > ApplicationsCsvService.MAX_DE_MINIMIS_AIDS:
        notes.append(f'Osa de minimis -tuista puuttuu raportilta')
    if application.pay_subsidies.count() > ApplicationsCsvService.MAX_PAY_SUBSIDIES:
        notes.append(f'Osa palkkatuista puuttuu raportilta')
    return ', '.join(notes)
...
class ApplicationsCsvService(CsvExportBase):
    ...
    def CSV_COLUMNS(self):
        columns = [
            ...
            CsvColumn("Työnantajan katuosoite (YTJ)", "official_company_street_address"),
            ...
            CsvColumn("Työnantajan osoite hakijalta?", "use_alternative_address", format_bool),
            CsvColumn("Työnantajan katuosoite (hakijalta)", "alternative_company_street_address"),
            CsvColumn("Työnantajan postinumero (hakijalta)", "alternative_company_postcode"),
            CsvColumn("Työnantajan postitoimipaikka (hakijalta)", "alternative_company_city"),
            CsvColumn("Työnantajan osasto", "company_department"),
            CsvColumn("Työnantajan yhtiömuoto", "company_form"),
            CsvDefaultColumn("Peruutuksen syy", "cancellation_reason"),
            CsvColumn("Työnantajan yhteyshenkilön etunimi", "company_contact_person_first_name"),
            CsvColumn("Työnantajan yhteyshenkilön sukunimi", "company_contact_person_last_name"),
            CsvColumn("Työnantajan yhteyshenkilön sähköpostiosoite", "company_contact_person_email"),
            CsvColumn("Työnantajan yhteyshenkilön puhelin", "company_contact_person_phone_number", str),
            CsvColumn("Yhdistys jolla yritystoimintaa?", "association_has_business_activities", format_bool),
            CsvColumn("Hakijan kieli", "applicant_language"),
            CsvColumn("Lähiesihenkilö-ruksi", "association_immediate_manager_check", format_bool),
            ...
            CsvDefaultColumn("Toinen palkkatukiprosentti", "additional_pay_subsidy_percent"),
            ...
            CsvDefaultColumn("Työntekijän kuukausipalkka (hakijalta)", "employee.monthly_pay"),
            CsvDefaultColumn("Työntekijän lomaraha (hakijalta)", "employee.vacation_money"),
            CsvDefaultColumn("Työntekijän muut kulut (hakijalta)", "employee.other_expenses"),
            ...
            CsvColumn("Työntekijä asuu Helsinkissä?", "employee.is_living_in_helsinki", format_bool),
            CsvDefaultColumn("Helsinki-lisän määrä lopullinen", "calculation.benefit_amount"),
            ...
            CsvDefaultColumn("Valtiotukimaksimi", "calculation.state_aid_max_percentage"),
            CsvDefaultColumn("Laskelman lopputulos", "calculation.calculated_benefit_amount"),
            CsvDefaultColumn("Manuaalinen syöttö", "calculation.override_benefit_amount"),
            CsvColumn("Manuaalinen syöttö kommentti", "calculation.override_benefit_amount_comment"),
            CsvColumn("Myönnetään de minimis -tukena?", "calculation.granted_as_de_minimis_aid", format_bool),
            CsvColumn("Kohderyhmätarkistus", "calculation.target_group_check", format_bool),
        ...
        for idx in range(self.MAX_PAY_SUBSIDIES):
            columns.extend([
                CsvDefaultColumn(f"Palkkatuki {idx + 1} / alkupäivä", nested_queryset_attr("pay_subsidies", idx, "start_date")),
                CsvDefaultColumn(f"Palkkatuki {idx + 1} / päättymispäivä", nested_queryset_attr("pay_subsidies", idx, "end_date")),
                CsvDefaultColumn(f"Palkkatuki {idx + 1} / palkkatukiprosentti", nested_queryset_attr("pay_subsidies", idx, "pay_subsidy_percent")),
                CsvDefaultColumn(f"Palkkatuki {idx + 1} / työaikaprosentti", nested_queryset_attr("pay_subsidies", idx, "work_time_percent")),
                CsvColumn(f"Palkkatuki {idx + 1} / vamma tai sairaus?", nested_queryset_attr("pay_subsidies", idx, "disability_or_illness", None), format_bool),
            ])

        for idx in range(self.MAX_DE_MINIMIS_AIDS):
            columns.extend([
                CsvColumn(f"De minimis {idx + 1} / myöntäjä", nested_queryset_attr("de_minimis_aid_set",idx,"granter")),
                CsvDefaultColumn(f"De minimis {idx + 1} / määrä", nested_queryset_attr("de_minimis_aid_set",idx,"amount")),
                CsvDefaultColumn(f"De minimis {idx + 1} / myönnetty", nested_queryset_attr("de_minimis_aid_set",idx,"granted_at")),
            ])
    ...
    def get_row_items(self):
        with translation.override("fi"):
            for application in self.get_applications():
                ...
                for application_row_idx, unused in enumerate(application.ahjo_rows or [None]):
