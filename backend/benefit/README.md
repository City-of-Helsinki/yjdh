...
Load test fixtures
    ...
    uv run manage.py loaddata default_terms.json
    uv run manage.py loaddata groups.json
    uv run manage.py loaddata test_applications.json
...
Set default permissions
    ...
    uv run manage.py set_group_permissions
...
- Run `uv run manage.py makemessages --ignore .venv -e xml,txt,html,py --no-location -l fi -l sv -l en`
- Run `uv run manage.py compilemessages --ignore .venv`
...
To seed the database with some mock application data, run `uv run manage.py seed`
, which by default generates 10 applications for each of the seven possible application statuses and one attachment with a .pdf-file for each of them. To generate more applications, use the optional `--number` flag, for example, running `uv run manage.py seed --number=30` creates 30 applications of each status. **Note that running the command deletes all previous application data from the database and clears the media folder.**
...
`$ uv run manage.py refresh_ahjo_token`
