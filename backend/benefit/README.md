...
See `.env.benefit-backend.example` for the  Minio variables and credentials.**Note** Switch from MinIO to the local disk by setting `USE_S3=0` before running the pytest tests or the seed command as MinIO cannot keep up with the files generated by these commands in the local container setup.
