...
describe('SalaryBenefitCalculatorView', () => {
  ...
  it.each([
    ...
    'updates %s via formik.setFieldValue when date input changes',
    async (fieldName) => {
      renderSubject();
  ...
  it('shows training compensation invalid warning when apprenticeship is on and add button is enabled', () => {
    ...
    expect(
      screen.queryByText(/koulutuskorvausrivin syöttö on vielä kesken/i)
    ).toBeInTheDocument();
  });
