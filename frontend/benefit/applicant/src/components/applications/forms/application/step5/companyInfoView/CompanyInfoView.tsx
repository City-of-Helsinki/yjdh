import AttachmentsListView
  from 'benefit/applicant/components/applications/forms/application/step5/attachmentsListView/AttachmentsListView';
              ...
              <$ApplicationDetailValue $column={1}>
                {data?.companyBusinessBrief &&
                  data?.companyBusinessBrief.split('\n').map((line) => (
                    <React.Fragment
                      key={`business-brief-${line.slice(0,20).replace(/\s/g, '')}`}
                    >
                      {line}
                      <br />
                    </React.Fragment>
