...
describe('AlterationForm', () => {
  ...
  it('renders common fields and hides suspension/einvoice specific fields by default', () => {
    ...
    expect(
      screen.getByRole('textbox', {
        name: new RegExp(
          t('applications.alterations.new.fields.endDate.label'),
          'i'
        ),
      })
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByRole('textbox', {
        name: new RegExp(
          t('applications.alterations.new.fields.resumeDate.label'),
          'i'
        ),
      })
    ...
    expect(
      screen.queryByRole('textbox', {
        name: new RegExp(
          t('applications.alterations.new.fields.einvoiceProviderName.label'),
          'i'
        ),
      })
    ).not.toBeInTheDocument();
  ...
  it('renders suspension and einvoice specific fields for matching values', () => {
    ...
    expect(
      screen.getByRole('textbox', {
        name: new RegExp(
          t(
            'applications.alterations.new.fields.einvoiceProviderIdentifier.label'
          ),
          'i'
        ),
      })
    ...
    expect(
      screen.getByRole('textbox', {
        name: new RegExp(
          t('applications.alterations.new.fields.einvoiceAddress.label'),
          'i'
        ),
      })
    ).toBeInTheDocument();
  ...
  it('returns true when date is inside handled recovery range', () => {
    ...
    expect(
      isDateInHandledRecoveryRange(application, new Date('2024-05-03'))
    ).toBe(true);
  });
  ...
  it('returns false when date is outside handled recovery range', () => {
    ...
    expect(
      isDateInHandledRecoveryRange(application, new Date('2024-05-06'))
    ).toBe(false);
  });
