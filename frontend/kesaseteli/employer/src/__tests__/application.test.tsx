...
import {
  expectAuthorizedReply, expectToGetApplicationErrorFromBackend,
  expectToGetApplicationFromBackend, expectToSaveApplication,
...
const waitForPageIsLoaded = async (): Promise<void> => {
  ...
  await waitFor(() => {
    expect(
      screen.queryByText(/form.loading/i)
...
const getApplicationPageApi = (queryClient: QueryClient, applicationId: Application['id']) =>
  ({
    replyOk: () => {
      ...
      return {
        expectations: {
          ...
          inputValueIsSet: async (inputName: keyof Application, value?: string): Promise<void> => {
            const inputValue = value ?? applicationFromBackend[inputName]?.toString();
            await waitFor(() => {
          ...
          inputHasError: async (inputName: keyof Application): Promise<void> => {
            await waitFor(() => {
              expect(screen.queryByText(new RegExp(`errors.${inputName}`, 'i'))).toBeInTheDocument();
            });
          },
        ...
        actions: {
          ...
          typeInvoicerEmail: (email: string) => typeInput('invoicer_email', email),
          typeInvoicerPhone: (phoneNumber: string) => typeInput('invoicer_phone_number', phoneNumber),
          clickContinueButton: (override: Partial<Application> = {}): void => {
            const updatedApplication: Application = {...applicationFromBackend, ...override};
    ...
    replyError: () => {
      ...
      expectations: {
        ...
        doesntDisplayCompanyData: async (): Promise<void> => {
          ...
          expect(screen.queryByText(company.business_id)).not.toBeInTheDocument();
          expect(screen.queryByText(company.industry)).not.toBeInTheDocument();
          expect(screen.queryByText(company.company_form)).not.toBeInTheDocument();
...
describe('frontend/kesaseteli/employer/src/pages/application.tsx', () => {
  ...
  describe('loading data', () => {
    ...
    describe('when authorized', () => {
      ...
      describe('when getting application data', () => {
        ...
        it('Should route to index page with default lang when applicaton id and locale is missing', async () => {
          ...
          await waitFor(() => expect(spyPush).toHaveBeenCalledWith(`${DEFAULT_LANGUAGE}/`));
        });

        it('Should route to index page with locale when applicaton id is missing', async () => {
          const locale: Language = 'en'
          const spyPush = jest.fn();
          renderPage(ApplicationPage, queryClient, { push: spyPush, query: {}, locale });
    ...
    it('updates application data when next button is clicked', async () => {
      ...
      await applicationPage.expectations.inputValueIsSet('invoicer_name');
      await applicationPage.expectations.inputValueIsSet('invoicer_email');
      await applicationPage.expectations.inputValueIsSet('invoicer_phone_number');
      ...
      applicationPage.actions.clickContinueButton({invoicer_name, invoicer_email, invoicer_phone_number});
      await applicationPage.expectations.allApiRequestsDone();
      await applicationPage.expectations.inputValueIsSet('invoicer_name', invoicer_name);
      await applicationPage.expectations.inputValueIsSet('invoicer_email', invoicer_email);
      await applicationPage.expectations.inputValueIsSet('invoicer_phone_number', invoicer_phone_number);
    });
    ...
    it('shows validation errors and disables continue button when missing values', async () => {
      ...
      await applicationPage.expectations.inputHasError('invoicer_phone_number');
