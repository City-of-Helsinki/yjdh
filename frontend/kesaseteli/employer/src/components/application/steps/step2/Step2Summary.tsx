...
const Step2Summary: React.FC = () => {
    ...
    const vouchers = React.useMemo(() => applicationQuery.data?.summer_vouchers || [], [applicationQuery.data]);
    ...
    }, [applicationQuery.isSuccess, vouchers, goToStep]);
    ...
    return (
        <ApplicationForm title={title} step={2}>
            <ApplicationSummary
                header={title}
                filterVoucherId={lastVoucher?.id}
            />
