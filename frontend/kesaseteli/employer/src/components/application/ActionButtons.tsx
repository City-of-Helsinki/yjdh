...
const ActionButtons = ({
  ...
  const { handleSubmit, formState: { isSubmitting, isValid } } = useApplicationForm();
  const {isLoading: isApplicationLoading} = useApplicationApi();
  const {handleStep,isFirstStep, isLastStep, previousStep, nextStep, isLoading: isWizardLoading} = useWizard();
  ...
  return (
    ...
    <Button
      ...
      disabled={!isValid || isSubmitting}
    >
      {isLastStep ? t(`common:application.buttons.send`) : t(`common:application.buttons.save_and_continue`)}
    </Button>
      </>
  );
};
