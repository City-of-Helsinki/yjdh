...
type YouthFormKey = keyof YouthFormData | 'school.name'
...
const getIndexPageApi = () => {
  ...
  return {
      expectations: {
        ...
        inputIsPresent: async (
          key: YouthFormKey,
        ): Promise<void> => {
          await screen.findByTestId(key);
        },
        inputIsNotPresent: async (
          key: YouthFormKey,
        ): Promise<void> => {
          expect(
        ...
        textInputIsValid: async (
          key: YouthFormKey,
        ): Promise<void> => {
          const input = await screen.findByTestId(key);
          expect(input).toBeValid();
        },
        schoolDropdownHasError: async (
          errorText: RegExp
        ): Promise<void> => {
          const input = await screen.findByRole('combobox', { name: /koulu/i })
          expect(input).toBeInvalid();
      ...
      actions: {
        ...
        clickSaveButton: async ({expectToPassValidation = true }: {expectToPassValidation: boolean}) => {
