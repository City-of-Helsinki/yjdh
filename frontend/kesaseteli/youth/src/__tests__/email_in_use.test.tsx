...
import EmailInUsePage from 'kesaseteli/youth/app/[locale]/email_in_use/page';
import renderComponent from 'kesaseteli-shared/__tests__/utils/components/render-component';
import React from 'react';
import { waitFor } from 'shared/__tests__/utils/test-utils';
import { DEFAULT_LANGUAGE } from 'shared/i18n/i18n';

describe('frontend/kesaseteli/youth/src/app/[locale]/email_in_use/page.tsx', () => {
  it('should not violate accessibility', async () => {
    const {
      renderResult: { container },
    } = renderComponent(await EmailInUsePage({ params: Promise.resolve({ locale: DEFAULT_LANGUAGE }) }));
  ...
  it('redirects to main page when clicking the button', async () => {
    ...
    renderPage(await EmailInUsePage({ params: Promise.resolve({ locale: DEFAULT_LANGUAGE }) }), { push: spyPush });
  ...
  it('shows default activation link expiration time (12 hours)', async () => {
    renderPage(await EmailInUsePage({ params: Promise.resolve({ locale: DEFAULT_LANGUAGE }) }));
