import {
  ...
  ATTACHMENT_TYPES,
} from '@frontend/shared/src/constants/attachment-constants';
import { DEFAULT_LANGUAGE, Language } from '@frontend/shared/src/i18n/i18n';
import {
  AttachmentType,
  KesaseteliAttachment,
} from '@frontend/shared/src/types/attachment';
import type User from '@frontend/shared/src/types/user';
import {
...
class FakeObjectFactory {
  ...
  public fakeContactInfo(): ContactInfo {
    return {
      ...
      payee_name: faker.name.findName(),
      payee_address: faker.address.streetAddress(),
      bank_swift_bic_code: faker.finance.bic(),
      bank_name: faker.company.companyName(),
      bank_address: faker.address.streetAddress(),
    };
  }
