...
import useYouthApplicationsListQuery from '../useYouthApplicationsListQuery';
...
describe('useYouthApplicationsListQuery', () => {
  ...
  it('serializes query params with multiple status values and ordering', async () => {
    nock(API_BASE_TEST_URL)
      .get(BackendEndpoint.YOUTH_APPLICATIONS)
      .query((queryObj) => {
        const statuses = Array.isArray(queryObj.status)
          ? queryObj.status
          : [queryObj.status];
        return (
